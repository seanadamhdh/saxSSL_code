...
## set pls tuning framework ####
### parameter grid ####
trainGrid<-expand.grid(
  ncomp=c(1:30)
)
...
for (i in var_list){
  ...
  for (set in set_list) {
    ...
    for (trans in trans_list){
      ...
      # somewhat equidistant (grouped) partiton, by each target variable
      ...
      if(paste0("pls_",set,"-",trans,"-",i) %in% done){
      ...
      if (!skip){
        ...
        ### save model ####
        saveRDS(out,file = paste0(root_dir,"/GitHub/R_main/R_main/temp",model_folder,"/pls_",set,"-",trans,"-",i))
